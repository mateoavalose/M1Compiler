...
PrefixParseFns:Dict = Dict[TokenType, PrefixParseFn]
InfixParseFns:Dict = Dict[TokenType, InfixParseFn]
...
class Parser:
    ...
    def _current_precedence(self) -> Precendece:
        assert self._current_token is not None
        try:
            return PRECEDENTES[self._current_token.token_type]
        except KeyError:
    ...
    def _expected_token_error(self, token_type:TokenType) -> None:
        assert self._peek_token is not None
        message = f'A {token_type} token was expected, but {self._peek_token.token_type} was found'
    ...
    def _parse_call(self, function:Expression) -> Call:
        assert self._current_token is not None
        call:Call = Call(self._current_token, function)
    ...
    def _parse_call_arguments(self) -> Optional[List[Expression]]:
        ...
        if self._peek_token.token_type == TokenType.RPAREN:
            ...
            return arguments
        self._advance_tokens()
        if expression := self._parse_expression(Precendece.LOWEST):
    ...
    def _parse_expression(self, precender:Precendece)->Optional[Expression]:
        ...
        try:
            ...
            message = f'Function to parse was not found {self._current_token.token_type}'
            self._errors.append(message)
            return None
        left_expression = prefix_parse_fn()
        assert self._peek_token is not None
        while not self._peek_token.token_type == TokenType.SEMICOLON and \
            precender < self._peek_precedence():
            try:
    ...
    def _parse_expression_statement(self) -> Optional[ExpressionStatement]:
        assert self._current_token is not None
        expression_statement:ExpressionStatement = ExpressionStatement(token = self._current_token)
        expression_statement.expression = self._parse_expression(Precendece.LOWEST)
        assert self._peek_token is not None
    ...
    def _parse_grouped_expression(self) -> Optional[Expression]:
        self._advance_tokens()
        expression = self._parse_expression(Precendece.LOWEST)
        if not self._expected_token(TokenType.RPAREN):
            return None
        return expression
    ...
    def _parse_function(self) -> Optional[Function]:
        assert self._current_token is not None
        function:Function = Function(token = self._current_token)
    ...
    def _parse_infix_expression(self, left:Expression) -> Infix:
        assert self._current_token is not None
        infix:Infix = Infix(token = self._current_token, operator = self._current_token.literal, left = left)
    ...
    def _parse_integer(self) -> Optional[Integer]:
        assert self._current_token is not None
        integer:Integer = Integer(token = self._current_token)
        try:
            integer.value = int(self._current_token.literal)
        except ValueError:
            message = f'Could not parse {self._current_token.literal} as integer'
            self._errors.append(message)
            return None
        return integer
    ...
    def _parse_let_statement(self) -> Optional[LetStatement]:
        ...
        if not self._expected_token(TokenType.IDENTIFIER):
            return None
        let_statement.name = self._parse_identifier() #Identifier(token = self._current_token, value = self._current_token.literal)
        if not self._expected_token(TokenType.ASSING):
            return None
        self._advance_tokens()
        let_statement.value = self._parse_expression(Precendece.LOWEST)
        assert self._peek_token is not None
        if self._peek_token.token_type == TokenType.SEMICOLON:
            self._advance_tokens()
    ...
    def _parse_prefix_expression(self) -> Prefix:
        assert self._current_token is not None
        prefix:Prefix = Prefix(token = self._current_token, operator = self._current_token.literal)
        self._advance_tokens()
        prefix.right = self._parse_expression(Precendece.PREFIX)
        return prefix
    
    def _parse_return_statement(self) -> Optional[ReturnStatement]:
        assert self._current_token is not None
        return_statement:ReturnStatement = ReturnStatement(token = self._current_token)
        self._advance_tokens()
        return_statement.return_value = self._parse_expression(Precendece.LOWEST)
        assert self._peek_token is not None
        if self._peek_token.token_type == TokenType.SEMICOLON:
            self._advance_tokens()
    ...
    def _peek_precedence(self) -> Precendece:
        assert self._peek_token is not None
        try:
            return PRECEDENTES[self._peek_token.token_type]
        except KeyError:
            return Precendece.LOWEST
